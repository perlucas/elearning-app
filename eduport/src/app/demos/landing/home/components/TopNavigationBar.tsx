...
const TopNavigationBar = () => {
  const { changeTheme, theme, appMenuControl } = useLayoutContext()
      ...
      <Container>
        ...
        <Dropdown>
          <DropdownToggle
            size="sm"
            variant="light"
            className="arrow-none lh-1 p-2 mb-0"
            id="bd-theme"
            type="button"
            aria-expanded="false"
          ...
          <DropdownMenu className="min-w-auto dropdown-menu-end" aria-labelledby="bd-theme">
            {themeModes.map((mode, idx) => (
              <li className={clsx({ 'mb-1': themeModes.length - 1 != idx })} key={idx}>
                <button
                  onClick={() => changeTheme(mode.theme)}
                  type="button"
                  className={clsx('dropdown-item d-flex align-items-center', { active: theme === mode.theme })}
