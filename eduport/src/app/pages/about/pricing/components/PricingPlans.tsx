...
import type { PricingPlanType, PricingType } from '@/types/other'
...
const PricingCard = ({ plan }: { plan: PricingPlanType }) => {
  const { features, price, title, badge, isRecommended } = plan
  return (
    <Card className="border rounded-3 p-2 p-sm-4 h-100">
      ...
      <CardBody className="pt-0">
        <ul className="list-unstyled mt-2 mb-0">
          {features.map((feature, idx) => {
            ...
            return (
              <li className="mb-3 h6 fw-light" key={idx}>
...
const PricingPlans = () => {
  const allPricing = useFetchData<PricingType[]>(getAllPricingPlans)
